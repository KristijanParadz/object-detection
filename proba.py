...
class ReIDModel:
    def __init__(self, device):
        ...
        self.model = nn.Sequential(*(list(base_model.children())[:-1]))  # output shape: [B, 2048, 1, 1]
        self.model.eval()
    ...
    def get_embedding(self, image):
        ...
        embedding = embedding.view(embedding.size(0), -1)  # flatten to [1, 2048]
    ...
    def process_frame(self):
        ...
        if results and results[0].boxes:
            for i, box in enumerate(results[0].boxes.xyxy):
                ...
                world_pos = self.transform_point([(x1 + x2) // 2, (y1 + y2) // 2])
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if detections[i]["camera_index"] != detections[j]["camera_index"]:
                    sim = cosine_similarity(detections[i]["embedding"], detections[j]["embedding"])
                    if sim > threshold:
...
if __name__ == "__main__":
    ...
    window = ObjectTrackingApp(video_paths, calibration_data, reid_model, device)
    window.show()
    sys.exit(app.exec())
